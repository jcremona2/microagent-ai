...
from .tracing import Tracer, StepType
    ...
    def _register_tools(self, tools: List[Union[Tool, Any]]) -> None:
        ...
        for tool in tools:
            ...
            if tool_instance.name in self.tools:
                if self.strict:
                    raise ValueError(f"Tool with name '{tool_instance.name}' already registered")
                logger.warning(f"Tool with name '{tool_instance.name}' already registered, skipping")
                continue
    ...
    def _execute_tool(self, tool_call: Dict[str, Any]) -> str:
        ...
        try:
            ...
            try:
                ...
                error = ToolExecutionError(f"Error executing tool '{tool_name}': {str(e)}")
